...
const SignUpForm = () => {
  ...
  const [emailSignUpPending, startEmailSignUpPending] = useTransition();
  const [googleSignUpPending, startGoogleSignUpPending] = useTransition();
  const [githubSignUpPending, startGithubSignUpPending] = useTransition();
  ...
  const isFormLoading =
    emailSignUpPending || googleSignUpPending || githubSignUpPending;

  function onSubmit(values) {
    const { firstName, lastName, ...formData } = values;
    startEmailSignUpPending(async () => {
      await signUp.email({
  ...
  const handleGoogleSignUp = () => {
    startGoogleSignUpPending(async () => {
      await signIn.social({
        provider: 'google',
        callbackURL: '/',
        fetchOptions: {
  ...
  const handleGithubSignUp = () => {
    startGithubSignUpPending(async () => {
      await signIn.social({
        provider: 'github',
        callbackURL: '/',
        fetchOptions: {
  ...
  return (
    <div className="flex-1 bg-card sm:px-10 px-5 py-10 flex flex-col justify-center relative">
      ...
      >
        <h2 className="text-xl font-semibold text-center">Sign Up Account</h2>
